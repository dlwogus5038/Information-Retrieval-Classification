...
word_wij_invert_nk = []
doc_dj_keyword_title_url_time_text = []
term_index_dict = {}
...
print("读取 doc_dj_keyword_title_url_time_text")
with open('temp\\doc_dj_keyword_title_url_time_text.json', 'r', encoding='utf-8-sig') as json_file:
    doc_dj_keyword_title_url_time_text = json.load(json_file)

print("开始搜索文档")
query_string = "王者荣耀实名认证"
...
for doc in query_doc_sim_dict: # 计算完整的 similarity
    query_doc_sim_dict[doc] = query_doc_sim_dict[doc] / (doc_dj_keyword_title_url_time_text[int(doc)]['dj'] * dq)

doc_list = sorted(query_doc_sim_dict.items(), key= lambda x:x[1] ,reverse=True)

classfy_num = 20
classfy_list = doc_list[0:classfy_num]
classfy_num = len(classfy_list)

print(classfy_list) # 相似度排序结果

keys = []
classfy_dict = {} # 关键词 - 文档编号
doc_vector_dict = {} # 文档编号 - 关键词 wij

for idx in range(0, classfy_num):
    keys = keys + doc_dj_keyword_title_url_time_text[int(classfy_list[idx][0])]['keywords']
    for word in doc_dj_keyword_title_url_time_text[int(classfy_list[idx][0])]['keywords']:
        if word not in ['\n',' ','\t', ' ']:
            index = term_index_dict[word]
            if word in classfy_dict:
                classfy_dict[word][idx] = word_wij_invert_nk[index]['wij'][int(classfy_list[idx][0])]
            else:
                classfy_dict[word] = [0] * classfy_num
                classfy_dict[word][idx] = word_wij_invert_nk[index]['wij'][int(classfy_list[idx][0])]
        else:
            print('word Error')
...
for idx in range(0, classfy_num):
    temp_list = []
    for word in classfy_dict:
        temp_list.append(classfy_dict[word][idx])
    doc_vector_dict[classfy_list[idx][0]] = temp_list
...
for idx in range(0, keys_nbr):
    ...
    center_vector[idx] = center_vector[idx] / classfy_num
...
print('计算每个文档之间的最短距离')
distance_check = [0] * (classfy_num * classfy_num)
stop_doc = []
count = 0
...
for id in center_distance_dict:
    if center_distance_dict[id] <= min_dist:
